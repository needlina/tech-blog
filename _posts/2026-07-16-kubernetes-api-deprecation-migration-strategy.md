...
description: "쿠버네티스에서 API(및 기능)의 폐기(deprecation) 주기를 점검하고, 실무에서 마이그레이션을 어떻게 접근하면 좋을지 정리해 보려고 합니다"
...
쿠버네티스에서 API(및 기능)의 폐기(deprecation) 주기를 점검하고, 실무에서 마이그레이션을 어떻게 접근하면 좋을지 정리해 보려고 합니다. 저는 아직 처음 다루는 입장에서 입장에서 하나씩 확인하면서 정리하는 방식으로 글을 쓰고 있어요. 읽는 분과 함께 "확인하며 남긴 메모", "처음 확인할 때 막히는 부분", 그리고 "운영 전 확인할 부분"를 단계적으로 적어 보겠습니다. 전문가처럼 단정적으로 말하기보다는, 제 경험과 공부한 내용을 바탕으로 조심스럽게 정리합니다. 틀릴 가능성이 있는 부분은 확정적으로 쓰지 않겠습니다.
...
![쿠버네티스 API 그룹과 버전 흐름을 단순화한 다이어그램 (원형으로 그룹-버전 전환 표시)](/assets/img/posts/blog/kubernetes-api-deprecation-migration-strategy/image-1.webp)
...
- CRD가 오래된 API를 의존하는 경우, CRD 제공자(오퍼레이터) 쪽에서 마이그레이션 기준가 있는지 확인해야 합니다.
...
- managed Kubernetes(GKE/AKS/EKS)에서는 컨트롤플레인 접근이 제한되므로, 제공되는 마이그레이션 기준나 지원 채널을 활용하세요.
...
![마이그레이션 워크플로(인벤토리 → 변환 → 테스트 → 배포)를 단계별 아이콘으로 표현한 일러스트](/assets/img/posts/blog/kubernetes-api-deprecation-migration-strategy/image-2.webp)
