...
Vue 3에서 reactive 객체를 통째로 갈아끼우거나 자식 props에 새 객체 참조를 계속 넘기면, 값은 거의 같아도 컴포넌트가 다시 렌더될 수 있습니다. 핵심은 **참조(identity)가 바뀌면 Vue가 다른 값으로 본다**는 점입니다. Devtools 렌더 카운트와 console 로그만으로도 원인을 꽤 빠르게 좁힐 수 있습니다.

이 글은 불필요 리렌더를 만드는 코드 패턴, 재현 절차, 그리고 실무에서 바로 적용하기 쉬운 수정 방법을 중심으로 정리합니다.
...
빠르게 테스트용 프로젝트를 만들려면 Vite가 편합니다. 아래는 로컬에서 바로 확인할 수 있는 최소 절차입니다.
...
정리하면, 먼저 Devtools에서 실제로 많이 렌더되는 컴포넌트를 찾고, 그 컴포넌트에 전달되는 props 참조가 매번 바뀌는지 확인하세요. 내용이 같은 객체를 계속 새로 만들고 있다면 속성 단위 수정, primitive 분해, toRef, shallowRef, markRaw 중 하나로 반응 범위를 줄이는 것이 우선입니다.
