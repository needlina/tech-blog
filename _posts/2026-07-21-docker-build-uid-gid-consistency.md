...
![Docker 빌드 후 이미지 내부 파일 소유자 확인 일러스트](/assets/img/posts/blog/docker-build-uid-gid-consistency/image-1.webp)
...
```bash
docker build --build-arg HOST_UID=$(id -u) --build-arg HOST_GID=$(id -g) -t myapp:local .
```
...
![권한 관련 실패 로그 ex) tar chown 오류)](/assets/img/posts/blog/docker-build-uid-gid-consistency/image-2.webp)
...
```bash
DOCKER_BUILDKIT=1 docker build --build-arg HOST_UID=$(id -u) --build-arg HOST_GID=$(id -g) -t myapp:test .
```

2. 실행(바인드 마운트):

```bash
docker run --rm -v $(pwd)/data:/app/data myapp:test sh -c "stat -c '%u:%g %n' /app/data/\*"
```

3. 실패 시 로그 수집:

```bash
docker run --rm -v $(pwd)/data:/app/data myapp:test sh -c "id -u && id -g && ls -l /app/data || true"
```

4. 호스트 측 확인:

```bash
ls -ln $(pwd)/data; stat -c '%u:%g %n' $(pwd)/data/\*
```
