---
title: "안전한 데이터베이스 마이그레이션 배포 기준: 실무 중심 체크포인트"
description: "데이터베이스 마이그레이션을 \"가능한 한 안전하게\" 배포하는 방법을 정리했습니다. 처음 다루는 입장에서 여러 자료를 찾아가며 운영에서 확인할 지점들을 중심으로 정리한 내용이라, 적용 전 확인할 기준과 예시입니다."
...
데이터베이스 마이그레이션을 "가능한 한 안전하게" 배포하는 방법을 정리했습니다. 처음 다루는 입장에서 여러 자료를 찾아가며 운영에서 확인할 지점들을 중심으로 정리한 내용이라, 적용 전 확인할 기준과 예시입니다.

확인하며 남긴 메모, 처음 확인할 때 막히는 부분, 그리고 운영에서 확인할 지점들을 중심으로 적어두었습니다. 코드나 명령어 예시도 포함했으니 환경에 맞게 응용해 보세요.
...
아래 예시는 실제로 따라해볼 수 있게 정리한 절차와 명령어들입니다. 환경에 따라 조정이 필요합니다.
...
```
{% endraw %}
가장 "migration이 실행되기 전에 백업이 자동으로 만들어지는가"와 "실패 시 알림 및 차단 로직이 있는가" 입니다.
...
하며
정리한 것은 "무조건 이 방법이 최고다"가 아니라, 실무에서 마주칠 위험을 줄이기 위한 체크리스트와 예시입니다. 환경(데이터베이스 종류, 버전, 트래픽 패턴)에 따라 적절히 조정해야 하고, 항상 복원 테스트를 먼저 해보는 습관이 중요하다고 느꼈습니다. 혹시 여러분이 실제로 실행해본 팁이나 다른 도구(Flyway, Liquibase, pt-online-schema-change, gh-ost 등)을 써본 경험이 있으시면 공유해주시면 좋겠습니다.
