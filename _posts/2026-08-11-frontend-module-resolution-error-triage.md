...
description: "webpack Vite Rollup에서 발생하는 모듈 해석 오류의 공통 원인과 단계별 점검 방법, 재현 명령어, 설정 파일 경로, ESM CJS 호환성 및 캐시 초기화 절차를 정리한 실무 가이드"
slug: "frontend-module-resolution-error-triage"
date: 2026-08-11 10:00:00 +0900
categories: ["Frontend"]
tags:
  ["javascript", "module-resolution", "webpack", "vite", "rollup", "빌드오류"]
image:
  path: /assets/img/posts/blog/frontend-module-resolution-error-triage/preview.png
  alt: "모듈 해석 오류 점검 썸네일"
---

모듈 해석 오류는 대부분 잘못된 import 경로, 확장자 미지정, alias/paths 누락, ESM/CJS 경계 문제, 빌드 도구 캐시 중 하나에서 시작됩니다. 먼저 오류 메시지를 경로, 확장자, alias, 모듈 형식으로 나눈 뒤 해당 빌드 도구의 resolve 설정과 Node 모듈 해석 규칙을 순서대로 확인하는 편이 가장 빠릅니다.

로컬에서는 통과하는데 CI에서만 깨지는 경우가 특히 번거롭습니다. 이 글은 그럴 때 바로 확인할 수 있도록 명령어, 설정 위치, 재현 순서를 중심으로 정리했습니다.
...
1. 오류 메시지 정확히 복사해 두기 — 빌드 로그 전체와 에러 스택(파일/라인) 확보
2. 로컬에서 최소 재현 명령 실행
...
이 문제는 설정을 한 번에 여러 개 바꾸면 오히려 원인을 놓치기 쉽습니다. 에러 로그를 저장하고, alias 하나 또는 extensions 하나처럼 변경 단위를 작게 잡은 뒤 빌드 결과를 비교하는 방식이 낫습니다. 특히 Windows나 macOS에서 통과하던 import 대소문자 문제가 Linux CI에서 터지는 경우가 많으니 파일명부터 실제 경로와 맞춰 보세요.
