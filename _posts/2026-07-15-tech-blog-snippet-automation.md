...
tags: ["jekyll", "코드스니펫", "자동화", "devops", "검증루프"]
image:
...
서문
저는 기술 블로그에 글을 쓰면서 코드 스니펫이 점점 쌓여가는 것을 경험했습니다. 그런데 시간이 지나면 옛날에 썼던 예제가 동작하지 않거나 의존성 충돌로 실패하는 일이 생기더군요. 그래서 최근에는 "블로그에 올린 코드가 항상 실행 가능한 상태"를 목표로 작은 자동화 파이프라인을 만들어 보고 있습니다. 이 문서는 정리한 내용과 실무에서 확인해볼 만한 포인트를 중심으로 적은 메모입니다. 아직 완벽하지 않을 수 있으니, 제안된 방법을 그대로 적용하기보다는 팀 환경에 맞게 조정하는 게 좋겠습니다.
