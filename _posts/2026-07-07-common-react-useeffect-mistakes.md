...
tags: ["react", "typescript", "useeffect", "실무패턴", "상태관리"]
---
...
서론
-------
useEffect는 React 함수형 컴포넌트에서 사이드 이펙트를 처리하는 표준 API입니다. 하지만 실무에서는 의도치 않은 리렌더·무한 루프·메모리 누수·경합(race) 등을 유발하는 잘못된 사용이 자주 보입니다. 이 문서는 대표적인 잘못된 패턴을 짚고, 현실적인 개선 방법과 코드 예제를 제공합니다. 모든 방법이 항상 최선은 아닐 수 있으니 각 패턴의 트레이드오프도 간단히 언급합니다.
