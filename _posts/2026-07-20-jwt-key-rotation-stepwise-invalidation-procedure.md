---
title: "JWT 키·알고리즘 롤링과 기존 토큰 단계적 무효화 실무 기준"
...
image:
  ...
  alt: "JWT 무중단 롤링 썸네일"
---
처음 이 주제를 접했을 때 저는 "키를 바꾸면 그냥 배포하면 되지?"라고 생각했는데, 실제로는 사용자에게 발급된 액세스 토큰과 리프레시 토큰의 만료 시간, 인증서 캐시(예: JWKS 캐시), 서명 알고리즘 변경 이슈 때문에 서비스 중단이나 보안 구멍이 생길 수 있다는 점이 헷갈렸습니다. 이 문서는 정리한 개념, 운영에서 바로 볼 지점, 실패 사례와 수정 예시, 검증 명령을 중심으로 단계별로 적어둔 초안입니다.
...
— 무엇을 먼저 확인해야 하나요, 언제 다른 선택을 고려하나요
