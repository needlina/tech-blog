...
서두는 요약만 짧게 적었고, 아래부터는 정리한 흐름대로 천천히 설명할게요. 실무에서 바로 확인하면 좋을 포인트와, 제가 처음 막히는 지점도 같이 적어뒀습니다.
...
- mock(인터페이스 수준 대체)과 factory(데이터/경량 서비스 대체)의 경계가 모호했는데, 목적에 따라 선택하면 됐습니다. 표준화된 기준가 없으면 팀마다 혼동이 올 수 있으니 이 부분은 문서로 남기면 좋아요.
...
위 예제에서 대체를 적용한 테스트가 `비즈니스 로직`을 검증하는 데 충분해야 한다는 것입니다. 외부 API의 실패 모드를 검증하려면 factory에서 실패 케이스도 만들면 됩니다.
