---
title: "Kubernetes 보안 진입점 변경 대응: PodSecurity와 seccomp 점검 기준"
description: "Kubernetes에서 보안 관련 정책이나 런타임 보안 설정(특히 PodSecurity 및 seccomp)으로 인해 애플리케이션이 동작하지 않거나 문제가 발생했을 때 어떤 점들을 확인하면 좋을지, 정리한 내용을 공유하려고 합니다"
slug: "k8s-podsecurity-seccomp-runtime-checklist"
date: 2026-07-16 12:00:00 +0900
categories: ["DevOps", "Security"]
tags: ["kubernetes", "podsecurity", "seccomp", "보안점검", "새소식", "트렌드"]
image:
  path: /assets/img/posts/blog/k8s-podsecurity-seccomp-runtime-checklist/preview.png
  alt: "Kubernetes 보안 변경 썸네일"
---

Kubernetes에서 보안 관련 정책이나 런타임 보안 설정(특히 PodSecurity 및 seccomp)으로 인해 애플리케이션이 동작하지 않거나 문제가 발생했을 때 어떤 점들을 확인하면 좋을지, 정리한 내용을 공유하려고 합니다. 저는 처음 다루는 입장에서 시선으로 하나씩 배우며 정리하고 있어서, 완벽한 정답을 제시하는 목적이 아니라 실무에서 점검할 수 있는 체크 포인트와 예시들을 함께 나누려 합니다.

![Kubernetes 네임스페이스 레이블과 PodSecurity 플로우를 단순히 보여주는 계층 구조 다이어그램 한 장](/assets/img/posts/blog/k8s-podsecurity-seccomp-runtime-checklist/image-1.webp)
...
![seccomp가 시스템 콜 레벨에서 동작하는 개념을 아이콘과 화살표로 단순히 설명한 일러스트 한 장](/assets/img/posts/blog/k8s-podsecurity-seccomp-runtime-checklist/image-2.webp)
