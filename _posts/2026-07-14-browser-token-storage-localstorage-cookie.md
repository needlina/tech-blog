...
브라우저에서 인증 토큰을 어디에 저장할지 고민하면서 정리한 내용을 공유하려고 합니다. 제가 이 주제를 처음 접했을 때는 "localStorage는 무조건 안 되고, 쿠키는 무조건 좋다" 같은 단순한 이야기만 들었는데, 공부하면서 보니 상황에 따라 장단점이 있고 실무에서는 확인해야 할 항목이 많이 있었습니다. 아래는 정리한 요점과, 처음 막히는 지점, 그리고 실무에서 점검하면 좋을 부분들을 중심으로 적어본 정리입니다. 환경에 맞게 다시 확인해야 합니다.
