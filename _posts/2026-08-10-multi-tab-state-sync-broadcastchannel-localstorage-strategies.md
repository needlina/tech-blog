...
로컬에서는 멀쩡한데 사용자가 같은 서비스를 여러 탭에 열어 두면 상태가 꼬이는 경우가 있습니다. 로그인 토큰 갱신, 장바구니 수정, UI 설정 저장처럼 "마지막으로 쓴 값"이 중요한 기능에서 자주 보입니다. 이 글은 **BroadcastChannel과 localStorage 이벤트를 같이 써서 탭 간 상태 충돌을 줄이는 방법**을 정리합니다.
...
## 문제를 구체적으로 재현하는 방법
검증은 별도 서버 코드 없이 같은 origin의 탭 두 개만 열어도 가능합니다.
...
Vue composable로 옮겨도 구조는 같습니다. 채널을 열고, message/storage 이벤트를 구독하고, unmount 시 정리하면 됩니다.
...
```js
// 탭 A 콘솔
bc.postMessage({ type: 'cart:update', payload: { items: ['x'], v: 2, ts: Date.now() } });
...
즉시 화면 갱신이 중요하면 BroadcastChannel을 먼저 쓰고, 새로고침이나 재시작 후 복구는 localStorage에 맡기는 식으로 역할을 나누면 됩니다.
...
마지막으로 개발자 도구에서 아래 순서대로 확인하면 원인을 빨리 좁힐 수 있습니다.
...
이 글의 코드는 탭 간 동기화 패턴을 설명하기 위한 최소 예제입니다. 실제 앱에서는 React Context, Zustand, Pinia, Vuex 같은 상태 저장소에 반영하는 부분과 서버 재동기화, 토큰 보안 정책을 함께 설계해야 합니다.
