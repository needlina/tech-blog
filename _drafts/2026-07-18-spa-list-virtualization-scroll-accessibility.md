...
## 나의 의견 1
여기에 직접 경험한 개선 사례(예: 어떤 페이지에서 overscan을 늘려 스크롤 부드러움을 얻었다 등)를 간단히 적어보세요.

## 나의 의견 2
여기에 직접 겪은 문제와 해결 과정을 적어 보세요. (예: 키보드 포커스가 튄 상황, 어떻게 고쳤는지 등)

이미지: 가상화 개념을 단순히 보여주는 일러스트
/ assets/img/posts/blog/spa-virtualization-scroll-accessibility/image-1.webp
이미지 출처: AI 생성 이미지

이미지: 키보드 포커스와 스크롤 보정을 설명하는 단순 일러스트
/ assets/img/posts/blog/spa-virtualization-scroll-accessibility/image-2.webp
이미지 출처: AI 생성 이미지
...
마무리하면서 조심스럽게 정리하자면, virtualization은 성능 문제를 해결하는 강력한 도구지만, 그로 인해 생기는 포커스·스크롤·접근성 문제들을 놓치면 사용성에 악영향을 줄 수 있습니다. 저는 실무에서 먼저 프로파일링으로 병목 원인을 확인하고, fixed-height로 단순화할 수 없을 때에만 variable-height로 복잡도를 늘리는 방식으로 접근하려 합니다. 궁금한 점이나 실제 코드 적용 사례가 있다면 이어서 같이 살펴볼게요.