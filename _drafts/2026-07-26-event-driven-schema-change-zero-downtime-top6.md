...
tags: ["postgresql", "event-driven", "schema-migration", "배포자동화", "스키마변경"]
image:
  path: /assets/img/posts/blog/event-driven-schema-change-zero-downtime-top6/preview.png
  alt: "이벤트 스키마 무중단 변경 썸네일"
---

구름이 조금 있는 날씨에 차분히 정리한 메모: 이벤트 기반 시스템에서 스키마 변경을 무중단으로 적용하려면, **(1) 호환성 규칙 준수(가역적 변경)**, **(2) 스키마 레지스트리와 호환성 모드**, **(3) 버전화된 이벤트 혹은 토픽 분리 + 듀얼 라이트·리플레이**, **(4) 업캐스터/어댑터로 소비자 적응**, **(5) 온라인 DB 변경(컬럼 추가→비어있음→백필→제약 추가 순)**, **(6) 사이드바이사이드(그림자) 리더 및 점진적 컷오버**를 조합하되, 각 전략별로 소비자 장애 로그, 레이턴시·오류율, DB 잠금 상태를 먼저 확인해야 합니다.

여기서는 각 전략의 목적과 왜 필요한지, 실무에서 먼저 보는 로그/명령, 실패 시 흔한 원인과 조치 순서를 중심으로 단계적으로 정리합니다. 공식 문서, 점검 명령, 예시 SQL/CLI, 오류 메시지 예시를 함께 적었습니다.

## 왜 이벤트 기반에서 스키마 변경이 더 복잡한가
이벤트 아키텍처는 데이터의 생산자(producer)와 소비자(consumer)가 느슨하게 결합되어 있다. 그래서 한 쪽의 스키마 변경이 다른 쪽의 역호환성 문제를 일으키기 쉽다. 특히 다음이 문제입니다.

- 소비자마다 배포 주기가 달라서 동시에 업데이트되지 않음.
- 이벤트 포맷이 바뀌면 직렬화(deserialization) 오류가 발생할 수 있음.
- DB 스키마 변경이 테이블 전체 재작성으로 장애를 유발할 수 있음(특히 대형 테이블).

내가 먼저 볼 로그는 소비자 애플리케이션의 deserialization 에러(예: "Unknown field", "AvroTypeException"), Kafka/메시지 브로커의 스키마 레퍼런스 오류, 그리고 DB에서의 긴 락 관련 로그입니다.
...
주의사항: "추가만 하면 안전"이 항상 성립하지 않습니다. 소비자가 필수값으로 기대하면 문제가 됩니다.
...
주의: PostgreSQL 버전에 따라 ADD COLUMN with DEFAULT가 전체 테이블 재작성할 수 있습니다. 운영 전 반드시 SELECT version()로 버전 확인하고 공식 문서(https://www.postgresql.org/docs/current/sql-altertable.html)를 확인하세요.
...
(이미지)
![그림자 리더와 기존 리더 결과 비교 그림](/assets/img/posts/blog/event-driven-schema-change-zero-downtime-top6/image-2.webp)
이미지 출처: AI 생성 이미지

## 전략 선택 기준 표
비교 기준을 먼저 정했습니다: 서비스 영향도(실시간 요구), 소비자 배포 속도, DB 테이블 크기, 메시지 형식(스키마 레지스트리 유무). 표 뒤에는 실제로 어떤 항목이 결정적일지 적습니다.
...
여기서 결정적 분기:
- DB가 크고 ALTER가 재작성된다면 온라인 DB 변경 + 백필 없이 NOT NULL을 강행하면 안 됩니다.
- 소비자 배포가 느리다면 호환성 규칙과 업캐스터 조합이 실무적으로 더 안전합니다.
...
여기서 시간을 많이 쓰는 곳은 업캐스터 테스트와 백필 배치 스크립트입니다.
...
## Q&A
다음 질문은 실제로 사람들이 검색할 가능성이 높은 질문을 골라 짧게 답했습니다.

## 자주 묻는 질문
Q: ALTER TABLE ADD COLUMN에 DEFAULT를 주면 항상 테이블 전체가 재작성되나요?  
A: PostgreSQL 버전과 DEFAULT 값의 종류에 따라 다릅니다. 운영 전 SELECT version()로 버전 확인하고 공식 문서(ALTER TABLE)에서 동작을 확인하세요.

Q: 스키마 레지스트리가 없는데 어떻게 호환성 보장하나요?  
A: 계약을 문서화하고 소비자별 통합 테스트를 CI에서 자동화하세요. 업캐스터/어댑터 패턴을 소비자 측에 둬서 점진 적용이 가능합니다.

Q: 듀얼 라이트 후 리플레이가 필요하면 어떻게 처리하나요?  
A: 메시지에 고유 ID를 포함하고 idempotent한 소비자를 구현한 뒤, 리플레이 시 중복 필터링을 적용하세요.

Q: 소비자가 최신 이벤트를 못 읽는 경우 먼저 어디를 보나요?  
A: 소비자 deserialization 에러 로그, 스키마 레지스트리 버전, 소스 토픽의 메시지 스키마(샘플)를 먼저 확인합니다.

Q: 대형 테이블의 NOT NULL 추가를 안전하게 처리하는 절차는?  
A: 컬럼 추가(널 허용) → 백필(배치) → DEFAULT 설정 → NOT NULL 설정 순으로 진행하고 pg_locks/pg_stat_activity로 모니터링하세요.

## 나의 의견 1
여기에 직접 적어보세요: 내 서비스의 가장 큰 테이블 이름과 현재 Postgres 버전은 무엇인가요? (예: orders, PostgreSQL 14.5)

## 나의 의견 2
여기에 직접 적어보세요: 이벤트 스키마를 관리하는 방법(레지스트리 사용여부, 스키마 형식), 그리고 소비자 배포 속도(빠름/느림)를 적어보세요.
...
끝으로, 이 토픽은 보통 한 번의 전략으로 끝나지 않습니다. 서비스 특성(소비자 수·배포 속도·DB 크기)에 맞게 위 전략을 조합하고, 먼저 작은 범위(예: 비핵심 토픽이나 소형 테이블)에서 연습하는 것을 권합니다. 다음으로 어떤 전략의 구체적 스크립트나 CI 테스트 예시를 원하면 어느 전략을 먼저 깊게 다룰지 알려주세요.