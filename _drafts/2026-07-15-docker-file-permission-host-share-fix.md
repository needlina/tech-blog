...
## 관련 이미지 주제
1. 호스트와 컨테이너 간 UID/GID 매핑을 보여주는 단순한 아이콘 기반 다이어그램 (호스트 사용자 → 숫자 UID → 컨테이너 파일 소유자)
2. 바인드 마운트와 named volume의 차이를 단순 그림으로 설명하는 일러스트 (호스트 폴더 연결 vs Docker 볼륨 컨테이너 연결)
