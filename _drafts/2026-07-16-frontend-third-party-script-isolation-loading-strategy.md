...
tags: ["third-party-scripts", "performance", "lazy-loading", "resource-priority", "observability"]
image:
...
## 관련 이미지 주제
1. "서드파티 스크립트 로드 플로우(비차단 → 지연 → 격리)를 간단한 아이콘으로 표현한 순서도"
2. "iframe 격리와 메인 문서 간 postMessage 통신을 화살표로 보여주는 단순 다이어그램"
