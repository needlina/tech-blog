...
(표는 모바일 가독성을 고려해 열 길이를 짧게 유지했습니다.)
...
## 나의 의견 1
내 환경(서비스명, Node 버전, 브로커 종류 등)은 무엇이었는지, 처음 실패한 테스트의 오류 메시지는 무엇인지, 수정 전후 로그 차이는 어땠는지 적어보세요.

## 나의 의견 2
구독 계약에서 나는 어떤 필드를 필수로 잡을지(예: orderId, createdAt, traceId), 실제로 얼마나 자주 스키마가 바뀌었는지 적어보세요.
...
![서비스 간 계약 테스트 구성도](/assets/img/posts/blog/graphql-subscription-contract-testing-for-event-consistency/image-2.webp)
이미지 출처: AI 생성 이미지