...
tags: ["package-manager", "ci-cache", "lockfile-format", "npm-pnpm-yarn", "news", "trend"]
image:
...
## 관련 이미지 주제
1. 패키지 매니저별(clipboard 스타일) 락파일과 캐시 흐름을 보여주는 단순한 다이어그램  
2. CI 파이프라인에서 락파일 해시를 키로 사용하는 캐시 전략을 설명하는 심플한 인포그래픽
