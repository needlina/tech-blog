...
async function handleMessage(msg) {
  ...
  try {
    ...
    if (isTransientError(err) && attempt < 5) {
      ...
      // DLQ로 이동: 실패사유와 함께
      channel.publish('', 'dead-letter-queue', Buffer.from(JSON.stringify({
        original: body,
        error: String(err),
        attempt
      })));
...
## 관련 이미지 주제
1. 메시지 큐와 데드레터 큐의 흐름을 보여주는 단순한 다이어그램(큐와 DLQ, 재시도 화살표 포함).
2. 재시도 정책(지수 백오프)을 설명하는 간단한 그래프(시도 횟수에 따른 지연 시간 증가를 선으로 표시).
