...
useRef는 렌더를 발생시키지 않는 가변 컨테이너, useState는 값이 바뀌면 컴포넌트를 다시 그리는 상태 저장 수단이라는 점을 먼저 기억하면 결정이 빨라집니다; 실무에서는 **UI 갱신이 필요한 값이면 useState, 단순한 식별자나 타이머 id, 이전 값 비교 등 렌더 필요 없는 저장이면 useRef**를 우선으로 고려하세요.

제가 정리한 기준은 React 18.2.0 기준 동작과 Node 18.x 환경에서 재현 가능한 예제, 콘솔 출력과 React DevTools 프로파일러로 검사하는 절차를 포함합니다.
...
이미지: 개념 다이어그램
![useRef와 useState의 동작 차이를 보여주는 단순 일러스트](/assets/img/posts/blog/react-useref-vs-usestate-mutable-values/image-1.webp)
이미지 출처: AI 생성 이미지
...
실패 사례와 고치는 예 (클로저 문제)
- 증상: setInterval 안에서 읽는 값이 초기값으로 고정되는 문제(스태일 클로저)
- 실패 코드:
```
function TimerBad() {
  const [count, setCount] = useState(0);
...
이미지: 개발자 도구 Profiler 사용 화면을 단순화한 일러스트
![React DevTools Profiler 사용을 나타내는 간단한 일러스트](/assets/img/posts/blog/react-useref-vs-usestate-mutable-values/image-2.webp)
이미지 출처: AI 생성 이미지
