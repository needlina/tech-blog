...
tags: ["serverless", "aws-lambda", "step-functions", "resource-cleanup", "배포자동화", "장애대응"]
image:
...
exports.handler = async function(event, context) {
  for (let i = 0; i < 10000; i++) {
    ...
    const timeoutId = setTimeout(() => ac.abort(), Math.min(remaining - 500, 5000));
...
나의 의견 1
- 이 섹션에는 위에서 설명한 패턴을 실제로 적용해 본 환경(런타임 버전, 초기 실패 메시지, 체크포인트 빈도 등)을 기록해 주세요. 예: "내 환경에서는 Node 18, Lambda timeout 120s, 첫 실패 로그: Task timed out after 120.00 seconds."

나의 의견 2
- 이 섹션에는 적용 전후의 로그 변화나 성능 지표(체크포인트 성공률, 평균 복구 시간 등)를 숫자로 적어 보세요. 예: "체크포인트 적용 전 1시간당 3건 누수, 후 0건."
...
![체크포인트와 재시도 흐름 일러스트](/assets/img/posts/blog/serverless-long-running-task-cancellation-pattern/image-2.webp)
이미지 출처: AI 생성 이미지

이미지 출처: AI 생성 이미지

읽어주셔서 감사합니다. 혹시 위에서 다룬 패턴 중에 현재 사용 중인 플랫폼(예: AWS Lambda, Google Cloud Functions, Cloud Run 등)과 작업 특성(길이, 외부 리소스 유형)을 알려주시면, 그 환경에 맞춘 체크포인트 전략과 재현 명령 예시를 같이 정리해 드릴게요.
