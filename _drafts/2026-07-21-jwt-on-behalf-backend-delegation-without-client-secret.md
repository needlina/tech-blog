...
두 번째 이미지: JWT assertion 예시와 서명 과정 일러스트
...
## 나의 의견 1
내 환경에서 사용한 인증서버 종류(제품/버전)는 무엇이었나요? 예: "Keycloak 21.0.1" 또는 "Auth0 - custom tenant" 등, 버전과 엔드포인트 구조를 적어보세요.

## 나의 의견 2
처음 실패했을 때 본 에러 메시지와, 수정 후 바뀐 로그/응답(예: HTTP 코드, error_description)을 짧게 적어보세요.
...
필요하시면 제 환경에서 했던 실패 로그(개인 정보 제거)와 curl 재현 명령을 공유할게요 — 어떤 점을 더 보고 싶으신가요?