...
- 본문은 최소 2500자 이상
- 본문 안에 글 주제와 직접 관련된 이미지 2개를 반드시 포함
- 이미지는 Markdown 이미지 문법 `![대체 텍스트](https://...)`으로 작성
- 각 이미지의 대체 텍스트는 이미지 내용과 글 주제를 설명하는 한국어 문장으로 작성
- 이미지는 도입부 이후 1개, 중간 설명 섹션 이후 1개를 배치
- 실제로 접근 가능한 공개 HTTPS 이미지 URL만 사용하고, 깨진 링크 가능성이 높은 임의의 로컬 경로나 placeholder 경로는 사용하지 말 것
