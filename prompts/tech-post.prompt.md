...
- 본문은 최소 2500자 이상
- 글 주제와 직접 관련된 이미지 주제 2개를 별도 섹션에 텍스트로만 작성
- 이미지 주제 섹션 제목은 `## 관련 이미지 주제`로 작성
- 이미지 주제는 번호 목록 2개만 작성하고, 각 항목은 짧고 구체적인 한국어 한 문장으로 작성
- 이미지 주제는 복잡한 화면, 코드, 글자, 실제 제품 UI, 세밀한 사진풍 장면이 아니라 단순한 기술 개념 일러스트에 어울리게 작성
- 본문에는 Markdown 이미지 문법, 외부 이미지 URL, 임의의 로컬 경로, placeholder 경로, 예시 경로, 이미지 마커를 작성하지 말 것
